...
async def test(message: Message, state: FSMContext):
    try:
        ...
        if firewall_status == firewall3.WHITE_ANCHOR:
            ...
            lpsql.insert("arttest_test1", [str(randint(0, 1000000000))+str(raw_unix()), message.from_user.id])
    ...
    try:
        ...
        if firewall_status == firewall3.WHITE_ANCHOR:
            ...
            try:
                ...
                await message.answer(str(lpsql.searchall("arttest_test1", "ID").count(message.from_user.id)))
                await state.clear()
            except KeyError:
                pass
